...
function initSearchBox(k, j, d, f, g) {
    ...
    Event.addListener(document, "modifiable", function () {
        ...
        Event.addSingleUseListener($(j), "focus", function () {
            var r;
            if (b) {
                r = b.getSelected();
                r = r.value || r.text
        ...
        Event.addListener($(f), "keydown", function (r) {
            if (r.keyCode == 13) {
                h(true)
            }
        })
    ...
    if (d) {
        ["click", "focus"].forEach(function (m) {
            Event.addListener($(d), m, function () {
                if (b) {
                    b.attach($(d), DropDownMenu.POSITION_BOTTOM_RIGHT, 100);
                    b.show()
                }
            })
        })
    ...
    g.forEach(function (an) {
        ...
        case "password":
            ...
            if (an.verify) {
                ...
                delayed(function () {
                    var v = new VerifyPassword($(an.verify), U, r)
                }, 0)()
            }
            break;
        case "copypaste":
...
App.prototype.init = function () {
    ...
    if (this.template_edit) {
        ...
        if (!k) {
            ...
            this.canvas.setEmptyMessage(f);
            Event.addListener($("idea"), "click", function () {
                q.select()
            });
